 ...
 */
   ...
   public void bmpToPels(String inFileName, int[] colorPallet, String name, Scanner scanner)
   {
       int i, j, k;
       int numberOfColors;
       int pel;
...
// System.out.printf("DEBUG: Color Table = %d, %06X\n", i, colorPallet[i]);
          ...
          for (i = 0; i < imageArray.length; i++)
          {
             ...
             {
                int currPel = imageArray[i][j];
                currPel = currPel & 0x00FFFFFF;
                bw.write((double)currPel/Math.pow(2,24) + "\n");
             }
          }
    ...
    public void pelsToBMP(String outFileName, String name)
    {
       /*
       ...
       try
       {
          ...
          int[][] outputs = new int[bmpInfoHeader_biHeight][bmpInfoHeader_biWidth];
...
// there is no color table for this true color image, so write out the pels
          ...
          for (i = 0; i < outputs.length; i++)
          {
             ...
             {
                outputs[i][j] = (int) (Double.parseDouble(br.readLine()) * Math.pow(2,24));
             }
          }
...
// The color table
      ...
      System.out.println("name of control file: ");
      ...
      try
      {
         ...
         String pels = controlscan.nextLine();
         String out = controlscan.nextLine();

         dibdumper.bmpToPels(inFileName, colorPallet, pels, controlscan);

         File file = new File(pels);
         try
         {
            ...
            for (i = 0; i < testerHiddenLayer.length; i++)
            {
         ...
         dibdumper.pelsToBMP(outFileName, out);
      }
      catch (FileNotFoundException e)
      {
         e.printStackTrace();
